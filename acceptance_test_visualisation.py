...
from envs.crowd_sim.policy.orca import ORCA
from envs.crowd_sim.visualization.observer_subscriber import notify
from envs.crowd_sim.visualization.video import Video
from envs.crowd_sim.visualization.plotter import Plotter
...
def main():
    ...
    print('It takes {} seconds to finish. Final status is {}'.format(env.global_time, info))
    
    if robot.visible and info == 'Reaching goal':
        print (info)
        human_times = env.get_human_times()
        #logging.info('Average time for humans to reach goal: %.2f', sum(human_times) / len(human_times))
        print('Average time for humans to reach goal: {}'.format(sum(human_times) / len(human_times)))
