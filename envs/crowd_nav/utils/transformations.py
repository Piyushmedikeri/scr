...
from envs.crowd_sim.utils.state import State, ObservableState, FullState
from envs.crowd_sim.utils.action import Action


def build_occupancy_map(human, other_agents, cell_num, cell_size, om_channel_size):
    u"""
    ...
    other_x_index[other_x_index < 0] = float(u'-inf')
    other_x_index[other_x_index >= cell_num] = float(u'-inf')
    other_y_index[other_y_index < 0] = float(u'-inf')
    other_y_index[other_y_index >= cell_num] = float(u'-inf')
    grid_indices = cell_num * other_y_index + other_x_index
    occupancy_map = np.isin(xrange(cell_num ** 2), grid_indices)
    if om_channel_size == 1:
        ...
        dm = [list() for _ in xrange(cell_num ** 2 * om_channel_size)]
        for i, index in np.ndenumerate(grid_indices):
            if index in xrange(cell_num ** 2):
...
def build_occupancy_map_torch(human, other_agents, cell_num, cell_size,
                        om_channel_size):
    u"""
    Builds the occupancy map centered around a human from the agents around it.
    :param human: the human with its states
...
def propagate(state, action, time_step, kinematics):
    if isinstance(state, ObservableState):
        ...
        raise ValueError(u'Type error')

    return next_state


def get_states_from_occupancy_map(occupancy_map, cell_num, cell_size, om_channel_size):
...
def propagate_occupancy_map(occupancy_map, state, action, time_step, kinematics, cell_num, cell_size, om_channel_size):
